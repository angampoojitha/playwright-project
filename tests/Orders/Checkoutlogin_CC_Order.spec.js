...
test('CheckoutLogin Reg CC order', async ({ page }) => {
...
const clickAndCollectRadio = page.getByText('Click & Collect');
if (!(await clickAndCollectRadio.isChecked())) {
  await clickAndCollectRadio.click(); 
}

const standardCC = page.getByText('Standard Click and Collect').first()
if (!(await standardCC.isChecked())) {
  await standardCC.click();
}

const addressInput = page.getByPlaceholder('Start typing address or postcode');
if (await addressInput.isVisible()) {
  await addressInput.fill('BL98RR');
  const store =  page.getByText('Your nearest collection point');
  await expect(store).toBeVisible();
  await page.getByRole('button', { name: 'Select' }).first().click();
}

const Customerdetails =  page.getByText('First Name');
if (await Customerdetails.isVisible()){
await page.locator('#firstName-id').fill('Jd');
  await page.locator('#lastName-id').fill('Test');
  await page.locator('#phone-id').fill('  ').fill('447382458688');
...
const OrderNumber = Orderid[1].trim();
